...
# the below are tests for the controller_spec page
describe "GET index" do
		it "should render an account page" do
			get :account_page
			expect(response).to render_template(:account_page)
		end
...
# is this a model or a controller spec test case?
		it "should be able to create and populate categories" do
			expect(@default) to create custom categories
		end
...
Per Ian, only really have code in the URL controller
...
Slug should NOT be nil

TDD style (write the test first, then write the code)
